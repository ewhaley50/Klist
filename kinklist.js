...
$(function () {
    ...
    $("#listType").change(function () {
        ...
        $.get(
            ...
            function (data) {
                ...
                // restore old selection after reload
    ...
    inputKinks = {
        ...
        init: function () {
            ...
            // Share / Copy link button (removes any old handlers too)
            $("#Export")
                .off("click")
                .on("click", function (e) {
                    e.preventDefault();
        ...
        // If anything still calls inputKinks.export(), make it copy the link
        export: function () {
            return inputKinks.copyShareLink();
        },

        // --- ORIGINAL IMGR EXPORT (kept, but renamed) ---
        ...
        drawCallHandlers: {
            ...
            kinkRow: function (context, drawCall) {
                ...
                for (var i = 0; i < drawCall.data.choices.length; i++) {
                    ...
                    var cx = 10 + drawCall.x + i * 20;
                    var cy = drawCall.y - 10;
        ...
        exportImgur: function () {
            ...
            $(".kinkCategory").each(function () {
                ...
                if (fields.length < 2) {
                    ...
                    titleCall.type = "titleSubtitle";
                    titleCall.data = { category: catName, fields: fields };
                }
                ...
                $cat.find(".kinkRow").each(function () {
                    ...
                    var rowCall = {
                        y: column.height,
                        type: "kinkRow",
                        data: { choices: [], text: $kinkRow.data("kink") }
                    };
                    column.drawStack.push(rowCall);
                    ...
                    $kinkRow.find(".choices").each(function () {
                        ...
                        rowCall.data.choices.push(selection);
                    });
                });
            });
            ...
            for (var c = 0; c < columns.length; c++) {
                if (tallestColumnHeight < columns[c].height) {
                    tallestColumnHeight = columns[c].height;
                }
            }
            ...
            for (var ci = 0; ci < columns.length; ci++) {
                var col = columns[ci];
                var drawStack = col.drawStack;

                var drawX = offsets.left + columnWidth * ci;
        ...
        encode: function (base, input) {
            ...
            for (var chunkId = 0; chunkId < numChunks; chunkId++) {
                ...
                while (inputIntValue > 0) {
                    var maxP = Math.floor(log(inputIntValue, hashBase));
                    var powVal = Math.pow(hashBase, maxP);
        ...
        decodeChunk: function (base, chunk) {
            ...
            for (var i = 0; i < chunk.length; i++) {
                var chr = chunk[i];
                var chrInt = inputKinks.hashChars.indexOf(chr);
                var pow = chunk.length - 1 - i;
                chunkInt += Math.pow(hashBase, pow) * chrInt;
            }

            var out = [];
            for (var p = inputPow - 1; p >= 0; p--) {
                var posBase = Math.floor(Math.pow(base, p));
        ...
        saveSelection: function () {
            ...
            $(".choice.selected").each(function () {
                ...
                selector =
                    "." + $(this).closest(".kinkCategory")[0].className.replace(/ /g, ".") + " " + selector;
        ...
        parseKinksText: function (kinksText) {
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                if (line[0] === "?") {
                    if (kink) kink.kinkDesc = line.substring(1).trim();
                }
            }
    ...
    (function () {
        ...
        inputKinks.inputPopup = {
            ...
            showIndex: function (index) {
                ...
                for (var n = 1; n <= inputKinks.inputPopup.numNext; n++) {
                    var nextKink = inputKinks.inputPopup.kinkByIndex(index + n);
                    var $nextKink = inputKinks.inputPopup.generateSecondary(nextKink);
                    $next.append($nextKink);
                    (function (skip2) {
                        $nextKink.on("click", function () {
                            inputKinks.inputPopup.showNext(skip2);
                        });
                    })(n);
                }
            },
